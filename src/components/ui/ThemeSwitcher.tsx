...
export function ThemeSwitcher({
  ...
  const { mode, setMode, accentColor, setAccentColor, toggleTheme, isDark } = useTheme();

  const themeModes: { value: ThemeMode; icon: React.ReactNode; labelKey: string }[] = [
  ...
  const accentColors = Object.entries(ACCENT_COLORS) as [AccentColor, { value: string; label: string }][];
  ...
  if (variant === 'icon-only') {
    return (
      <button
        ...
        aria-label={t(isDark ? 'settings.themeLight' : 'settings.themeDark')}
          ...
          <div className="accent-selector">
            {accentColors.map(([colorKey, { value }]) => (
              <button
                ...
                aria-label={t(`settings.color${colorKey.charAt(0).toUpperCase() + colorKey.slice(1)}`)}
                title={t(`settings.color${colorKey.charAt(0).toUpperCase() + colorKey.slice(1)}`)}
              />
            ))}
          </div>
        )}
